...
public class GraphicsGrid extends JPanel
{
	...
	protected synchronized void paintComponent(Graphics g) {
		...
		{
		   g.drawLine(i + relWidth, relHeight, i + relWidth, pixels*(int)Math.floor(this.height/this.pixels) + relHeight);
		}

		for (int i = 0; i < pixels*(int)Math.floor(this.height/this.pixels); i += pixels) 
		{
		   g.drawLine( relWidth, i + relHeight, pixels*(int)Math.floor(this.width/this.pixels) + relWidth,i + relHeight);
		}
